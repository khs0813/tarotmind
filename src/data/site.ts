...
export function isSiteUrlConfigured(): boolean {
  ...
  return Boolean(raw && !raw.includes('your-domain.com') && /^https?:\/\//i.test(raw));
}

export function getSiteUrl(): string {
  return (getRawSiteUrl() || 'http://localhost:4321').replace(/\/$/, '');
}

export function getContactEmail(): string {
  return import.meta.env.PUBLIC_CONTACT_EMAIL ?? 'hello@example.com';
}
